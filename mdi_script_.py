...
# ## Overview
#
# The purpose of this script is to take Medicaid Claim Level Data received in various formats and standardize into
# one format suitable for database consumption.
...
# curr_dir = 'C:/Users/ramtekea/Documents/cld_converter_v5_latest'
curr_dir = r'\\ridnsw002-svm03a\swp01$\Home\VD10-NEW\agnihoa6.RNUMDMAS\Documents\MDI_PREPROCESS\\'
            #r'\\ridnsw002-svm03a\swp01$\Home\VD10-NEW\agnihoa6.RNUMDMAS\Documents\MDI_PREPROCESS
# curr_dir = os.getcwd()
startTime = datetime.now()
# PMRootDir = os.environ['PMRootDir']
sender = "agnihoa6@gene.com"
mdi_ops_receivers = "agnihoa6@gene.com"
mdi_cci_receivers = "agnihoa6@gene.com"
msg = MIMEMultipart()
msg['From'] = sender
...
if run_parameter == 'CONVERTER':
    log_success = open(curr_dir + r'\outputs\logs\\' + date.today().strftime("%Y%m%d") + '_converter_success.txt', 'a')
...
if os.path.isfile(curr_dir + r'\outputs\logs\\' + date.today().strftime("%Y%m%d") + '_validator.txt'):
    os.remove(curr_dir + r'\outputs\logs\\' + date.today().strftime("%Y%m%d") + '_validator.txt')
log_validation = open(curr_dir + r'\outputs\logs\\' + date.today().strftime("%Y%m%d") + '_validator.txt', 'a')
...
if adhoc_trigger == 'Y':
    state_cats = pd.read_excel(r'adhoc\config_files\mdi_adhoc_config_setup.xlsx', header=[1])
    state_cats['head_lvl'] = state_cats['head_lvl'].fillna(0)
    adhoc_map_overrides = pd.read_excel(r'adhoc\config_files\mdi_adhoc_state_map_overrides.xlsx')
    adhoc_map_overrides.columns = adhoc_map_overrides.columns.str.upper()
else:
    state_cats = pd.read_excel(curr_dir + r'\inputs\config_files\mdi_cld_config_setup.xlsx', header=1)
    state_cats['head_lvl'] = state_cats['head_lvl'].fillna(0)
...
# process quarter
if adhoc_trigger == 'Y':
    process_qtr_df = pd.read_excel(r'adhoc\config_files\mdi_adhoc_config_setup.xlsx', header=None)
    process_qtr = process_qtr_df.iloc[0, 1]
else:
    process_qtr_df = pd.read_excel(curr_dir + r'\inputs\config_files\mdi_cld_config_setup.xlsx', header=None)
    process_qtr = process_qtr_df.iloc[0, 1]

# conversion factors by state & conversion exception lists
convert_factor = pd.read_excel(curr_dir + r'\inputs\config_files\mdi_conversion_factor.xlsx',
...
convert_factor_exceptions = pd.read_excel(curr_dir + r'\inputs\config_files\mdi_conversion_program_exception.xlsx',
                                          header=0,
...
cf_exceptions_config_lvl = cf_exceptions_config_lvl[['STATE', 'PROG_NAME', 'LABELER_NAME', 'INVC_QTR', 'RBT_QTR',
                                                     'EXCEPT_TYPE', 'CONVERSION_EXCEPT']].drop_duplicates()

# NEED TO ADD HEAD LEVEL TO PROGRAM CONFIG OVERRIDES########################################

# program or labeler level mapping overrides
mapping_overrides = pd.read_excel(curr_dir + r'\inputs\config_files\mdi_cld_program_map_override.xlsx',
...
def qtr_assess(x, y):
    ...
    try:
        ...
        for key, value in list(qtr_parse_dict.items()):
        ...
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Quarter assessment error for ' +
        # ' ' + curr_file_path + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
                                               '%m/%d/%Y %H:%M:%S') + ' Quarter assessment error for ' + ' ' +
                             curr_file_path + str(repr(e)) + '\n')
...
def ndc_assess(x, y, z):
    ...
    try:
        ...
        for key, value in list(ndc_parse_dict.items()):
        ...
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' NDC file level assessment error
        # for ' + ' ' + curr_file_path + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
                                               '%m/%d/%Y %H:%M:%S') + ' NDC file level assessment error for ' + ' ' +
                             curr_file_path + str(repr(e)) + '\n')
...
def unit_convert(x, y, z):
    ...
    try:
        ...
        '''print(
            ('Unit conversion error (could not convert units) for: ' + curr_file_path + ' with error ' + str(repr(e))))
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Unit conversion error (could not
        # convert units) for ' + curr_file_path + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
                                               '%m/%d/%Y %H:%M:%S') + 'Unit conversion error (could not convert '
                                                                      'units) for ' + curr_file_path + str(
            repr(e)) + '\n')'''
...
"""
        ...
        with open(curr_dir + r'\outputs\logs\\' + process_qtr + '_cld_validation.csv') as input_file:
    ...
    if len(cld_validation_success) > 0 and (run_parameter == 'CONVERTER'):
        success_email = open(
            curr_dir + r'\outputs\status_email\\' + date.today().strftime("%Y%m%d") + '_success_upload_email.txt', 'w')
        success_email.write('Hi Team,' + '\n' + '\n')
        success_email.write('Below files were uploaded succesfully: ' + '\n' + '\n')
        for file in cld_validation_success.itertuples():
            success_email.write(file.STATE + ' : ' + file.FILE_NAME + '\n')
        success_email.write('\n' + '***** This is an automated alert from MDI Cld Validator *****' + '\n')
...
mag_ffs_read_cols = pd.read_excel(curr_dir + r'\inputs\file_format_mapping_sets\magellan_ffs.xlsx')
mag_mco_read_cols = pd.read_excel(curr_dir + r'\inputs\file_format_mapping_sets\magellan_mco.xlsx')
std_read_cols = pd.read_excel(curr_dir + r'\inputs\file_format_mapping_sets\standard.xlsx')
...
qtr_parse_dict = {'MULTI': '([1-2]{1}[0-9]{3}[0-9]{1}[q,Q][1-4]{1})',
                  ...
                  'YYQN': r'(\D[0-9]{2}[Q,q][1-4]{1}\D)',
                  'NQYY': r'(\D[1-4]{1}[Q,q][0-9]{2}\D)',
                  '/YYYYN/': r'(\D[1-2]{1}[0-9]{3}[1-4]{1}\D)',
                  'QNYY': r'(\D[Q,q][1-4]{1}[0-9]{2}\D)',
                  'NYYYY': r'(\D[1-4]{1}20[0-9]{2}\D)',
...
col_spec_dict = {
    ...
    'MS_COL_SPEC':[(0, 11), (11,12), (13, 25), (26,27), (30-35), (35,36), (37,46), (47,51), (52,55),
                    (56,63), (64-71), (72,75), (76,88), (89,100), (101,111), (112,122), (122,123),(124,134),(135,142)]}

# create custom headers dict ###

if adhoc_trigger == 'Y':
    header_file_list = glob.glob(r'adhoc\adhoc_mapping_sets\HEADER\*.*')
else:
    header_file_list = glob.glob(curr_dir + r'\inputs\state_mapping_sets\HEADER\*.*')
...
if adhoc_trigger == 'Y':
    state_mapping_list = glob.glob(r'adhoc\adhoc_mapping_sets\*.*')
else:
    state_mapping_list = glob.glob(curr_dir + r'\inputs\state_mapping_sets\STD\*.*')
...
if adhoc_trigger == 'Y':
    mag_mco_map = pd.read_excel(r'adhoc\adhoc_mapping_sets\MAG\cust_mag_mco.xlsx')
    mag_ffs_map = pd.read_excel(r'adhoc\adhoc_mapping_sets\MAG\cust_mag_ffs.xlsx')
    alt_mag_mco_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_mco_alternate.xlsx')
    alt_mag_ffs_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_ffs_alternate.xlsx')
else:
    mag_mco_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_mco.xlsx')
    mag_ffs_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_ffs.xlsx')
    alt_mag_mco_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_mco_alternate.xlsx')
    alt_mag_ffs_map = pd.read_excel(curr_dir + r'\inputs\state_mapping_sets\MAG\mag_ffs_alternate.xlsx')
...
if adhoc_trigger == 'Y':
    ...
    state_file_list = glob.glob(r'adhoc\to_process\*\*\*\*\*\*.*')
#     state_file_list = glob.glob("\\\\dnafiles1\sales\MCOps\Gov't Pgms\Medicaid\STATES\CLD\MDI Directory Structure\*\*\*\*\*\*.*")
    ...
    state_file_list = glob.glob(curr_dir + r'\inputs\to_process\*\*\*\*\*\*.*')
...
if len(state_file_list) > 0:
    ...
    for file_path in state_file_list:
        ...
        parsed_config_list.append(dict(list(zip(cols, vals))))
    ...
    try:
        ...
        # changes from here
        config_table1 = pd.DataFrame()
        config_table1['EXCLUDE_FILE'] = None
        config_table1['EXCLUDE_DESCRIP'] = None
        config_table1[['EXCLUDE_FILE', 'EXCLUDE_DESCRIP']] = config_table.apply(lambda x: exclude_file(x, process_qtr),
                                                                                axis=1)
        final_config_table = pd.concat([config_table, config_table1], axis=1, join='inner')

        config_table_all = final_config_table.copy()

        final_config_table.to_excel(curr_dir + '/temp/last_run_config_table_all.xlsx')

        excluded_rbt_qtr = final_config_table[final_config_table['RBT_QTR'] == 'UNK']
        excluded_rbt_qtr[
            'EXCLUDE_DESCRIP'] = 'Rebate Quarter Issue :Either No or Duplicate Rebate Quarters present in the file ' \
                                 'name. '

        excluded_ndc = final_config_table[final_config_table['NDC'] == 'UNK']
        ...
        excluded_actv_config = final_config_table[final_config_table['ACTIVE_CONFIG'] != 'Y']
        ...
        final_config_table = final_config_table[final_config_table['RBT_QTR'] != 'UNK']
        final_config_table = final_config_table[final_config_table['FILE_FORMAT'] != 'UNK']
        final_config_table = final_config_table[final_config_table['NDC'] != 'UNK']
        final_config_table = final_config_table[
            final_config_table['INVC_QTR'] == process_qtr]  # limit processing to configured invoice quarter
        final_config_table = final_config_table[final_config_table['ACTIVE_CONFIG'] == 'Y']

        final_config_table.to_excel(curr_dir + r'\temp\last_run_config_table.xlsx')
        excluded_files.to_excel(curr_dir + r'\temp\last_run_excluded.xlsx')

    except Exception as e:
        print(e)
        final_config_table = pd.DataFrame()
        # log_success.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Did not build config table- no
        # files to process' + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
        ...
        if cld_file.FILE_CONVERT_TYPE == 'CSV':
            ...
            if alt_mag_trigger == 'Y':
                df = pd.read_excel(cld_file.FILE_PATH
                                   , header=4
                                   , dtype=str
                                   , sheet_name=use_mag_sheet)  # , skipinitialspace=True)
            else:
                df = pd.read_excel(cld_file.FILE_PATH
                                   , header=int(cld_file.HEAD_LVL)
                                   , dtype=str)  # , skipinitialspace=True)
...
def cld_read_special(df_dict, cld_file, read_result, error_dict):
    ...
    try:
        ...
        for cld_sheet, df in list(df_dict.items()):
        ...
        # log_validation.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Read (special) error with '
        # + cld_file.FILE_NAME + ' [' + sheet + ']: ' + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
...
def cld_map(df_dict, cld_file, error_dict):
    ...
    try:
        ...
        for sheet, df in list(df_dict.items()):
...
def cld_map_col_test(df_map, cld_file, map_set, map_result, map_desc, error_dict):
    ...
    if cld_file.MAG_TYPE == 'STD':
        if 'ORIGINAL_ICN_TCN' in list(state_map_dict[map_set].values()):
    ...
    try:
        ...
        for sheet, df in list(df_map.items()):
            ...
            if len(missing_req_cols) > 0:
                ...
                for column in res:
                    for key, value in list(state_map_dict[map_set].items()):
                        if column == value:
    ...
    try:
        ...
        for sheet, df in list(df_map.items()):
            ...
            if cld_file.STATE == 'CA':
                ...
                df['PROVIDER_QUANTITY'] = df['INVOICE_QUANTITY']
        ...
        log_validation.write(datetime.strftime(datetime.now(),
            ...
            repr(e)) + '\n')
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Standardize error with ' +
        # cld_file.FILE_NAME + ' [' + sheet + ']: ' + str(repr(e)) + '\n')
...
def cld_unit_convert(df_standardized, cld_file, error_dict):
    ...
    try:
        ...
        for sheet, df in list(df_standardized.items()):
            ...
            if cld_file.USE_CONVERT_FILE != 'Y' and cld_file.CONVERSION_EXCEPT == 'Y':
                ...
                current_file_config.columns = final_config_table.columns
        ...
        if not bool(converted_standardized_dict):
        ...
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Standardize (conversion) error
        # with ' + cld_file.FILE_NAME + ' [' + sheet + ']: ' + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
...
def cld_finalize(df_standardize_dict, cld_file, error_dict):
    ...
    try:
        ...
        for sheet, df in list(df_standardize_dict.items()):
        ...
        # log_error.write(datetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Finalize error with ' +
        # cld_file.FILE_NAME + ' [' + sheet + ']: ' + str(repr(e)) + '\n')
        log_validation.write(datetime.strftime(datetime.now(),
...
def cld_test(df_finalize_dict, map_set, cld_file, error_dict):
    ...
    for sheet, df in list(df_finalize_dict.items()):
    ...
    if not bool(df_finalize_dict):
        ...
        try:
            ...
            for sheet, df in list(df_finalize_dict.items()):
                ...
                if not os.path.isfile(curr_dir + r'\outputs\process_working\\' + file_output_name + '.csv'):
                    df.to_csv(curr_dir + r'\outputs\process_working\\' + file_output_name + '.csv', index=False)
                else:
                    df.to_csv(curr_dir + r'\outputs\process_working\\' + file_output_name + '.csv', index=False,
                              mode='a',
                              header=False)
            ...
            archive_directory = cld_file.DIR_PATH + r'\_archived\\'
            new_archived_name = datetime.strftime(datetime.now(), '%m_%d_%Y_%H_%M_%S' + '_' + cld_file.FILE_NAME)
            archive_name = cld_file.DIR_PATH + r'\\' + new_archived_name
...
cld_summary_cols = list(final_config_table.columns.values)
...
existing_process_files = glob.glob(
    curr_dir + r'\outputs\process_working\*.*')  # remove existing files from temporary process folder
for f in existing_process_files:
    os.remove(f)

error_dict = {'read_error': {}, 'map_error': {}, 'standardize_error': {}, 'finalize_error': {}, 'test_error': {}}
...
for cld_file in config_table_all.itertuples():
    ...
    current_cld_file.columns = list(final_config_table.columns.values)
...
if len(cld_run_summary_build) > 0:
    ...
    try:
        ...
        cld_run_summary_full['RUN_DATE'] = datetime.now().strftime("%m/%d/%Y %I:%M:%S")
        ...
        cld_run_summary_full.to_excel(curr_dir + r'\temp\last_run_summary_full.xlsx', index=False)
        cld_run_summary.to_excel(curr_dir + r'\temp\last_run_summary.xlsx', index=False)
        ...
        if run_parameter == 'CONVERTER':
            ...
            if not os.path.isfile(curr_dir + r'\outputs\logs\\' + process_qtr + '_summary_full.csv'):
                cld_run_summary_full.to_csv(curr_dir + r'\outputs\logs\\' + process_qtr + '_summary_full.csv'
                                            , index=False)
            ...
            if not os.path.isfile(curr_dir + r'\outputs\logs\\' + process_qtr + '_summary.csv'):
                cld_run_summary.to_csv(curr_dir + r'\outputs\logs\\' + process_qtr + '_summary.csv'
                                       , index=False)
                ...
                cld_run_summary.to_csv(curr_dir + r'\outputs\logs\\' + process_qtr + '_summary.csv'
                                       ...
                                       , mode='a'
                                       , header=False)
        if not os.path.isfile(curr_dir + r'\outputs\logs\\' + process_qtr + '_cld_validation.csv'):
            cld_validation.to_csv(curr_dir + r'\outputs\logs\\' + process_qtr + '_cld_validation.csv'
                                  , index=False)
        else:
            cld_validation.to_csv(curr_dir + r'\outputs\logs\\' + process_qtr + '_cld_validation.csv'
                                  , index=False)
...
processing_dir = curr_dir + r'\outputs\process_working\\'
processed_dir = curr_dir + r'\outputs\processed\\'
...
if run_parameter == 'CONVERTER':
    for f in processed_files:
        ...
        try:
            ...
            # log_validation.write(da tetime.strftime(datetime.now(), '%m/%d/%Y %H:%M:%S') + ' Moving file ' + f + '
            # to target ' + processed_dir + '\n')
            print(('Moving ' + f))
